...
public class GlobalExceptionHandler {
    ...
    protected ResponseEntity<ApiError> handleMeetNotFoundException(NoSuchElementException ex, WebRequest request) {
        List<String> errors = Collections.singletonList(ex.getMessage());
        return handleExceptionInternal(ex, new ApiError(errors), new HttpHeaders(), HttpStatus.NOT_FOUND, request);
    }

    @ExceptionHandler(EmptyThresholdsException.class)
    protected ResponseEntity<ApiError> handleEmptyThresholdsException(EmptyThresholdsException ex, WebRequest request) {
        List<String> errors = Collections.singletonList(ex.getMessage());
